...
public abstract class ApplicationTestCase {
    ...
    protected void assertResponse(String endpoint, Integer expectedStatusCode, String expectedResponse,
        ...
        final ResultMatcher response = expectedResponse.isEmpty() ? content().string("") :
            content().json(expectedResponse);

        this.mockMvc.perform(get(endpoint).headers(headers)).andExpect(status().is(expectedStatusCode)).andExpect(response);
    }
    ...
    protected void givenISendEventsToTheBus(DomainEvent<?>... domainEvents) {
        this.eventBus.publish(Arrays.asList(domainEvents));
    }
}
